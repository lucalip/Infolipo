...
def damier2(): #fonction dessinant le tableau
       ligne_vert()
       ligne_hor() 
...
def click_gauche2(event): #fonction rendant vivante la cellule cliqu̩e donc met la valeur 1 pour la cellule cliqu̩e au dico_case
    x = event.x -(event.x%c)
    y = event.y -(event.y%c)
...
def click_droit2(event): #fonction tuant la cellule cliqu̩e donc met la valeur 0 pour la cellule cliqu̩e au dico_case
    x = event.x -(event.x%c)
    y = event.y -(event.y%c)
    can1.create_rectangle(x, y, x+c, y+c, fill='white')
    dico_case[x,y]=0

def change_vit2(event): #fonction pour changer la vitesse(l'attente entre chaque ̩tape)
    global vitesse
...
i=0
while i!= width/c: #assigne une valeur 0(morte) a chaque coordonn̩es(cellules) (valeur par d̩fault en quelque sorte ^^)
    j=0
    while j!= height/c:
        x=i*c
        y=j*c
